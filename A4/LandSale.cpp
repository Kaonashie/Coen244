...
LandSale::LandSale(std::string address, Customer customer, int agent_id, Date listing_date, Date sell_date,
				   int sold_price, double area_of_land, int sale_price): Property(address, customer, agent_id, listing_date, sell_date, sold_price), area_of_land(area_of_land), sale_price(sale_price) {}
double LandSale::get_area_of_land() const {
	return this->area_of_land;
}
int LandSale::get_sale_price() const {
...
LandSale::~LandSale() {
	std::cout << "LandSale object destroyed successfully.\n";
}
